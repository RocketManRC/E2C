...
PinWindow* pin_window_create(PinWindowCallbacks callbacks, char *s) {
  ...
  if (pin_window) {
    ...
    if (pin_window->window) {
      ...
      text_layer_set_text(pin_window->main_text, s);
      ...
      text_layer_set_text(pin_window->sub_text, "Enter your data to continue");
      ...
      for (int i = 0; i < PIN_WINDOW_NUM_CELLS; i++) {
        selection_layer_set_cell_width(pin_window->selection, i, 22);
      }
